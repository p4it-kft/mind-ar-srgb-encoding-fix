import { Scene as m, WebGLRenderer as y, SRGBColorSpace as w, PerspectiveCamera as g, Mesh as M, MeshStandardMaterial as b, Group as p, BufferGeometry as x, BufferAttribute as A } from "three";
import { CSS3DRenderer as S } from "three/addons/renderers/CSS3DRenderer.js";
import { C as R } from "./controller-sLTuLJIQ.js";
import { U as E } from "./ui-2_N98-vS.js";
const C = { BufferGeometry: x, BufferAttribute: A };
class I {
  constructor({
    container: a,
    uiLoading: t = "yes",
    ...
    disableFaceMirror: d = !1
  }) {
    this.container = a, this.ui = new E({ uiLoading: t, uiScanning: o, uiError: n }), this.controller = new R({
      filterMinCF: e,
      filterBeta: h
    }), this.disableFaceMirror = d, this.scene = new m(), this.cssScene = new m(), this.renderer = new y({ antialias: !0, alpha: !0 }), this.cssRenderer = new S({ antialias: !0 }), this.renderer.outputColorSpace = w, this.renderer.setPixelRatio(window.devicePixelRatio), this.camera = new g(), this.userDeviceId = s, this.environmentDeviceId = c, this.anchors = [], this.faceMeshes = [], this.latestEstimate = null, this.container.appendChild(this.renderer.domElement), this.container.appendChild(this.cssRenderer.domElement), this.shouldFaceUser = !0, window.addEventListener("resize", this._resize.bind(this));
  }
  async start() {
  ...
  addFaceMesh() {
    const a = this.controller.createThreeFaceGeometry(C), t = new M(a, new b({ color: 16777215 }));
  ...
  _startAR() {
    return new Promise(async (a, t) => {
      ...
      this.container, this.controller.onUpdate = ({ hasFace: e, estimateResult: h }) => {
        ...
        if (e) {
          const { metricLandmarks: s, faceMatrix: c, faceScale: d, blendshapes: l } = h;
  ...
  _resize() {
    ...
    let h, s;
    const c = e.videoWidth / e.videoHeight, d = n.clientWidth / n.clientHeight;
    c > d ? (s = n.clientHeight, h = s * c) : (h = n.clientWidth, s = h / c), e.style.top = -(s - n.clientHeight) / 2 + "px", e.style.left = -(h - n.clientWidth) / 2 + "px", e.style.width = h + "px", e.style.height = s + "px", this.shouldFaceUser && !this.disableFaceMirror ? e.style.transform = "scaleX(-1)" : e.style.transform = "scaleX(1)";
    const l = a.domElement, r = t.domElement;
    l.style.position = "absolute", l.style.top = e.style.top, l.style.left = e.style.left, l.style.width = e.style.width, l.style.height = e.style.height, r.style.position = "absolute", r.style.top = e.style.top, r.style.left = e.style.left, r.style.transformOrigin = "top left", r.style.transform = "scale(" + h / parseFloat(r.style.width) + "," + s / parseFloat(r.style.height) + ")";
  }
}
window.MINDAR || (window.MINDAR = {});
window.MINDAR.FACE || (window.MINDAR.FACE = {});
window.MINDAR.FACE.MindARThree = I;
export {
  I as MindARThree
};
